...
class cfgr final {
  static constexpr auto address =
      reinterpret_cast<volatile uint32_t *>(0x40021004);
...
public:
  ...
  void set_sysclk_source(const sysclk src) { *address |= (unsigned(src) << 1); }
...
class cfgr2 final {
  static constexpr auto address =
      reinterpret_cast<volatile uint32_t *>(0x4002102C);
...
class ahbenr final {
  static constexpr auto address =
      reinterpret_cast<volatile uint32_t *>(0x40021014);
